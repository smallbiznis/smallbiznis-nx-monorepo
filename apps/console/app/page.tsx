...
// Mock Billing Client (replace with actual import)
class BillingClient {
  ...
  customers = {
    list: async () => {
      ...
      return [
        { id: "cus_1", name: "Acme Corp", email: "contact@acme.com", created_at: "2024-12-01" },
        { id: "cus_2", name: "Beta Inc", email: "hello@beta.com", created_at: "2024-12-02" },
      ];
    }
  };
  ...
  invoices = {
    list: async () => {
      return [
        { id: "inv_1", invoice_no: "INV-ORG-0001", customer_id: "cus_1", total_amount: 125000, currency: "USD", status: "paid", issued_at: "2024-12-07" },
        { id: "inv_2", invoice_no: "INV-ORG-0002", customer_id: "cus_2", total_amount: 89000, currency: "USD", status: "open", issued_at: "2024-12-06" },
        { id: "inv_3", invoice_no: "INV-ORG-0003", customer_id: "cus_3", total_amount: 210000, currency: "USD", status: "paid", issued_at: "2024-12-05" },
      ];
    }
  };
  ...
  subscriptions = {
    list: async () => {
      return [
        { id: "sub_1", customer_id: "cus_1", price_id: "price_1", status: "active" },
      ];
    }
  };
}
...
const useBillingStore = create((set, get) => ({
  ...
  // Client
  client: null as BillingClient | null,
  
  // Data
  organizations: [
    { id: "acme-corp", name: "Acme Corp", role: "Owner", currency_code: "USD", slug: "acme" },
  ...
  stats: {
    ...
    invoices: { value: 156, total: "$45.2K" },
    usage: { value: "2.4M", unit: "events" }
  },
  ...
  setActiveOrg: (orgId: string) => {
    set({ activeOrg: orgId, showOrgSwitcher: false });
    get().initClient(orgId);
  },
  
  setActiveTab: (tab: string) => set({ activeTab: tab }),
  toggleOrgSwitcher: () => set((state: any) => ({ showOrgSwitcher: !state.showOrgSwitcher })),
  
  // Initialize Client
  initClient: (orgId: string) => {
    const org = get().organizations.find((o: any) => o.id === orgId);
    if (!org) return;
  ...
  fetchData: async () => {
    ...
    try {
      ...
      // Format invoices
      const formattedInvoices = invoices.slice(0, 3).map((inv: any) => ({
        id: inv.id,
        customer: customers.find((c: any) => c.customer_id === inv.customer_id)?.name || "Unknown",
        amount: `$${(inv.total_amount / 100).toLocaleString()}`,
        status: inv.status,
        date: new Date(inv.issued_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })
      }));
...
export default function BillingDashboard() {
  ...
  const currentOrg = organizations.find((o: any) => o.id === activeOrg);

  // Initialize client on mount
  useEffect(() => {
    initClient(activeOrg);
  }, []);
